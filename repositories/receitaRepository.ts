...
export const receitaRepository = {
  async findManyPaginated(
    ...
    paginacao: Paginacao,
  ): Promise<PaginaResultado<Receita>> {
  ...
  async findById(id: string, userId: string) {
    return prisma.receita.findFirst({ where: { id, userId }, include: { categoria: true } });
  },

  async create(data: CreateLancamentoInput): Promise<Receita> {
    return prisma.receita.create({ data });
  },

  async createMany(data: CreateLancamentoInput[]): Promise<Receita[]> {
    return prisma.$transaction(data.map((item) => prisma.receita.create({ data: item })));
  },

  async update(id: string, userId: string, data: UpdateLancamentoInput): Promise<Receita> {
    const existente = await prisma.receita.findFirst({ where: { id, userId }, select: { id: true } });
    if (!existente) {
      throw new Error("Receita não encontrada para este usuário.");
    }
    return prisma.receita.update({ where: { id: existente.id }, data });
  },

  async delete(id: string, userId: string): Promise<void> {
    const existente = await prisma.receita.findFirst({ where: { id, userId }, select: { id: true } });
    if (!existente) return;
    await prisma.receita.delete({ where: { id: existente.id } });
  },

  async deleteFuturasByGrupo(grupoRecorrenciaId: string, userId: string, aPartirDe: Date): Promise<number> {
    const result = await prisma.receita.deleteMany({
  ...
  async somaPorPeriodo(userId: string, dataInicio: Date, dataFim: Date, categoriaId?: string): Promise<number> {
    const result = await prisma.receita.aggregate({
